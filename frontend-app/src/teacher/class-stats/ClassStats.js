...
const ClassStatsTeacher = () => {
  ...
  useEffect(() => {
    ...
    if (!email) {
      setError('Teacher email not found.');
      return;
    }
  ...
  const columnOptions = {
    ...
    title: { text: '📊 Grade Distribution', style: baseStyle.style },
    xAxis: {
      categories: completeHistogram.map(h => h.grade.toString()),
      title: { text: 'Grade', ...baseStyle },
      labels: baseStyle,
    },
    yAxis: {
      title: { text: 'Student Count', ...baseStyle },
      labels: baseStyle,
    },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 to-slate-900 text-slate-100 px-4 py-12">
      <div className="max-w-5xl mx-auto space-y-12">
        <h1 className="text-4xl font-light tracking-wide text-center text-white-force">📈 Class Statistics</h1>

        <div className="bg-slate-800/60 backdrop-blur-lg rounded-xl shadow-lg max-w-xl mx-auto px-6 py-8">
          <h2 className="text-xl font-semibold text-white-force mb-4">Course Selection</h2>
          <label htmlFor="class-select" className="text-sm text-white-force block mb-2">Select a course:</label>
          <select
            id="class-select"
        ...
        {stats && (
          <div style={{ display: 'flex', justifyContent: 'center', gap: '1rem', marginTop: '2rem' }}>
            ...
            { label: 'Maximum', value: stats.max, icon: '⬆️' }].map(({ label, value, icon }) => (
              <div
                key={label}
                style={{
                  display: 'inline-flex',
        ...
        {selectedClass && histogram.length > 0 && (
          <div
            ...
            <div style={{ backgroundColor: '#1e293b', padding: '1rem', borderRadius: '1rem', width: '400px', height: '300px', boxShadow: '0 4px 12px rgba(0,0,0,0.2)' }}>
